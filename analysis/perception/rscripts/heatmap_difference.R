...
########################################
base_output_dir <- "../graphs/heatmap_differences"
dir.create(base_output_dir, recursive = TRUE, showWarnings = FALSE)

# Store difference data and track scales per response
...
for (response_key in names(response_types)) {
  ...
  for (comparison_key in names(comparison_pairs)) {
    ...
    minuend_data <- group_plot_data_list[[minuend_key]]
    subtrahend_data <- group_plot_data_list[[subtrahend_key]]

    if (is.null(minuend_data) || is.null(subtrahend_data)) {
      warning(paste(
        "Skipping comparison", comparison_key,
        "because one of the age groups has no data"
      ))
      next
    }
