...
processed_data <- processed_data %>%
  mutate(age_cat = factor(ifelse(age >= MIMUM_OLD_AGE, "old", "not-old"),
                          levels = c("old", "not-old")))

fit_brms_model <- function(data) {
  brm(
    formula = response ~ svot * age_cat + sf0 * age_cat + (1 + svot + sf0 | subject),
    data = data,
...
model <- save_or_load_model(
  model_path = "../model/all_ages_multinomial_logistic_regression_old_not_old.rds",
  data = processed_data,
