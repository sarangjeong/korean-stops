...
fit_brms_model <- function(data) {
  brm(
    formula = response ~ svot + sf0 + svot:poa + sf0:poa + svot:sage + sf0:sage + (1 + svot + sf0 | subject),
    data = data,
...
model <- save_or_load_model(
  model_path = "../model/all_ages_multinomial_logistic_regression_poa_interaction.rds",
  data = processed_data,
...
forest_plot <- tidy_model_filtered %>%
  ...
  scale_x_discrete(limits = c("VOT", "f0", "Labial", "Dorsal", "Age:VOT", "Age:f0")) +  # Fixed order
